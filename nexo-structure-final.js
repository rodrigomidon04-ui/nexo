...
(function(){
  ...
  function prepare(){
    ...
    workspace.innerHTML=`<div class="container"><div class="nexo-workspace-head"><div><div class="tag">04 — Mi Nexo</div><h2>Conectividad</h2></div><p class="muted">Sesiones, contactos, invitaciones y ecosistema.</p></div><div class="nexo-workspace-card" id="nexoWorkspaceCard"><button type="button" class="nexo-final-link" data-final-target="sesiones"><span>◌</span><strong>Sesiones</strong><em>Conversaciones, actividad e historial.</em><i>›</i></button><button type="button" class="nexo-final-link" data-final-target="contactos"><span>◎</span><strong>Contactos</strong><em>Tu red de personas conectadas.</em><i>›</i></button><button type="button" class="nexo-final-link" data-final-target="invitaciones"><span>✉</span><strong>Invitaciones</strong><em>Salas e invitaciones pendientes.</em><i>›</i></button><button type="button" class="nexo-final-link" data-final-target="ecosistema"><span>✦</span><strong>Ecosistema</strong><em>Work, Edu, Care y futuros espacios NEXO.</em><i>›</i></button></div></div>`;
    ...
    if(nav){
      ...
      menu.querySelectorAll('[data-final-menu]').forEach(b=>b.addEventListener('click',e=>{e.preventDefault();close();const key=b.dataset.finalMenu;if(key==='sesiones'){center(miCentro);return;}const sec=$(key);if(sec){sec.hidden=false;sec.style.display='';center(sec);}}));
    }
  }
