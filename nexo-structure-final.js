...
(function(){
  ...
  function prepare(){
    ...
    workspace.innerHTML=`<div class="container"><div class="nexo-workspace-head"><div><div class="tag">04 — Mi Nexo</div><h2>Conectividad</h2></div><p class="muted">Sesiones, contactos, invitaciones y ecosistema.</p></div><div class="nexo-workspace-card" id="nexoWorkspaceCard"><button type="button" class="nexo-final-link" data-final-target="sesiones"><span>◌</span><strong>Sesiones</strong><em>Conversaciones, actividad e historial.</em><i>›</i></button><div class="nexo-final-body" data-final-body="sesiones"></div><button type="button" class="nexo-final-link" data-final-target="contactos"><span>◎</span><strong>Contactos</strong><em>Tu red de personas conectadas.</em><i>›</i></button><div class="nexo-final-body" data-final-body="contactos"></div><button type="button" class="nexo-final-link" data-final-target="invitaciones"><span>✉</span><strong>Invitaciones</strong><em>Salas e invitaciones pendientes.</em><i>›</i></button><div class="nexo-final-body" data-final-body="invitaciones"></div><button type="button" class="nexo-final-link" data-final-target="ecosistema"><span>✦</span><strong>Ecosistema</strong><em>Work, Edu, Care y futuros espacios NEXO.</em><i>›</i></button><div class="nexo-final-body" data-final-body="ecosistema"></div></div></div>`;

    // Put the existing Contacts, Invitations and Ecosystem modules inside 04 — Mi Nexo.
    [contactos,invitaciones,ecosistema].forEach(sec=>{
      if(sec){sec.hidden=false;sec.style.display='';}
    });
    const bodyContactos=workspace.querySelector('[data-final-body="contactos"]');
    const bodyInv=workspace.querySelector('[data-final-body="invitaciones"]');
    const bodyEco=workspace.querySelector('[data-final-body="ecosistema"]');
    if(contactos&&bodyContactos)bodyContactos.appendChild(contactos);
    if(invitaciones&&bodyInv)bodyInv.appendChild(invitaciones);
    if(ecosistema&&bodyEco)bodyEco.appendChild(ecosistema);
    ...
    if(nav){
      ...
      const targetBodies={sesiones:workspace.querySelector('[data-final-body="sesiones"]'),contactos:bodyContactos,invitaciones:bodyInv,ecosistema:bodyEco};
      // Sessions opens the existing Mi Centro module; the other three expand their existing modules inside 04.
      menu.querySelectorAll('[data-final-menu]').forEach(b=>b.addEventListener('click',e=>{
        ...
        const body=targetBodies[key];
        if(body){
          body.classList.add('open');
          const sec=body.querySelector('section');
          if(sec){sec.hidden=false;sec.style.display='';}
          requestAnimationFrame(()=>center(workspace));
        }
      }));

      workspace.querySelectorAll('[data-final-target]').forEach(btn=>btn.addEventListener('click',()=>{
        const key=btn.dataset.finalTarget;
        if(key==='sesiones'){center(miCentro);return;}
        const body=targetBodies[key];
        if(body){body.classList.toggle('open');const sec=body.querySelector('section');if(sec){sec.hidden=false;sec.style.display='';}if(body.classList.contains('open'))requestAnimationFrame(()=>center(workspace));}
      }));
    }
  }
